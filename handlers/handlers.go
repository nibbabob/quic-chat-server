...
func HandleSecureConnection(conn types.Connection, connID string) {
	...
	for {
		...
		default:
			...
			// *** THIS IS THE FIX: Restore the call to the stream handler ***
...
func handleSecureJoin(_ context.Context, stream types.Stream, conn types.Connection, connID string, msg types.Message) {
...
func handleSecureMessage(_ context.Context, stream types.Stream, connID string, msg types.Message) {
	server.Mutex.RLock()
...
func IsConnectionClosed(err error) bool {
	...
	// *** THE FIX: Remove the overly generic string check. ***
